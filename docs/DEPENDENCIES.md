...
Dependabot opens at most one routine version-update pull request per ecosystem.
Routine npm, Python and Docker minor/patch updates are grouped into ecosystem
batches; GitHub Actions updates are grouped together. Automatic major-version
PRs are ignored because runtime/toolchain major changes need an explicit planned
upgrade with compatibility research, lock regeneration and the full applicable
gate. Security updates remain eligible and are grouped separately for npm and
Python. Operators may open a focused major-upgrade PR when the project chooses
to expand support; closing a generated PR does not establish support.
