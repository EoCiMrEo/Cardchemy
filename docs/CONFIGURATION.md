...
The API receives AI_PROVIDER_ENABLED and public availability/budget settings.
Only the generation worker receives AI_API_KEY or GEMINI_API_KEY. Enabling a
...
| Setting and template default | Purpose, bounds, and required condition |
| --- | --- |
| AI_PROVIDER_ENABLED=false; AI_PROVIDER=gemini | Generation admission switch and gemini/openai_compatible profile. Disabled allows startup without a key. |
| AI_MODEL=gemini-3.8-flash | Model identifier, 1–128 characters; verify availability with the provider. |
| AI_API_KEY=empty; GEMINI_API_KEY=empty | Worker-only credential and legacy Gemini fallback. Required when the selected provider needs authentication; avoid duplicate keys. |
| AI_BASE_URL=empty | Required for openai_compatible; must be an HTTP(S) origin/endpoint without embedded credentials, query, or fragment. Empty is correct for native Gemini. |
| AI_ALLOW_UNSTABLE_MODEL=false | Explicit production opt-in for model names indicating preview/latest/experimental status. |
| AI_TEMPERATURE=0.2; AI_MAX_OUTPUT_TOKENS=8192; AI_CONTEXT_WINDOW_TOKENS=1048576 | Sampling 0–2; per-call output 64–131072 tokens; model context 2048–4194304 tokens. |
| AI_PROVIDER_TIMEOUT_SECONDS=90; AI_PROVIDER_MAX_RETRIES=3 | Per-call timeout 1–600 seconds; 0–3 retries after the first attempt. |
| AI_RETRY_BASE_SECONDS=3; AI_RETRY_MAX_SECONDS=30 | Provider retry delay bounds, 3–60 and 3–600 seconds; respect usable longer Retry-After hints. |
| AI_CONCURRENCY=3 | Provider calls shared across one worker process, 1–32. Multiple worker replicas need divided quotas or a distributed governor. |
| AI_REQUESTS_PER_MINUTE=5; AI_INPUT_TOKENS_PER_MINUTE=250000 | Configured provider rate tier, 1–100000 requests/minute and 1–100000000 input tokens/minute. |
| AI_RATE_LIMIT_SAFETY_PERCENT=80 | Allowed fraction of configured RPM/TPM, 1–100 percent. |
| AI_CHUNK_INPUT_TOKENS=1200; AI_CHUNK_OVERLAP_TOKENS=120 | Grounded logical chunk target 128–131072 tokens; overlap 0–32768 tokens. |
| AI_REQUEST_INPUT_TARGET_TOKENS=40000; AI_CARDS_PER_REQUEST=10 | Packed request target 2048–4000000 input tokens; 1–100 cards per call. |
| AI_SUMMARY_OUTPUT_TOKENS=1024; AI_REFILL_ROUNDS=2 | Per-summary budget 64–32768 tokens; 0–5 bounded refill rounds. |
| AI_MAX_JOB_INPUT_TOKENS=200000; AI_MAX_JOB_OUTPUT_TOKENS=262144 | Whole-job ceilings, 1024–20000000 input and 64–5000000 output tokens. |
| AI_DUPLICATE_SIMILARITY_THRESHOLD=0.88 | Near-duplicate rejection threshold, 0.5–1. |
| AI_INPUT_COST_PER_MILLION_USD=0; AI_OUTPUT_COST_PER_MILLION_USD=0 | Operator-entered USD per million tokens, 0–10000; zero disables monetary estimate. |
| AI_MAX_ESTIMATED_COST_USD=5 | Preflight per-job cost ceiling, greater than 0 and at most 100000 USD. |

AI budget settings also have cross-field checks: chunk overlap is smaller than
