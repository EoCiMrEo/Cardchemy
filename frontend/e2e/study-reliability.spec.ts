...
test('locks a rapid double activation across the timer boundary to one progress write', async ({ page }) => {
  ...
  let observeSession: () => void = () => {}
  const sessionStarted = new Promise<void>((resolve) => { observeSession = resolve })
  let releaseProgress: () => void = () => {}
  const progressRelease = new Promise<void>((resolve) => { releaseProgress = resolve })
  let observeProgress: () => void = () => {}
  const progressStarted = new Promise<void>((resolve) => { observeProgress = resolve })
  const api = await installMockApi(page, {
    auth: 'student',
    resolver: async (call) => {
      if (call.method === 'GET' && call.path === '/study/sets/set-1/session') {
  ...
  await page.goto('/study/set-1')
  // Let authentication and lazy-route hydration finish, then freeze time before
  // releasing the session response that mounts the one-second question timer.
  ...
  await expect(page.getByRole('button', { name: 'Next Question' })).toHaveCount(0)
  // Cross the original one-second deadline while the first write is pending.
  await page.clock.runFor(200)
  expect(api.count('POST', '/study/progress')).toBe(1)
  expect(api.callsFor('POST', '/study/progress')[0]?.body).toEqual({
    flashcard_id: studyCards[0].id,
    selected_option: 'Mitochondrion',
  })
  await expect(page.getByRole('button', { name: 'Next Question' })).toHaveCount(0)
