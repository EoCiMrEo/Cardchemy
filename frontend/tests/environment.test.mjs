...
test('one root file, process precedence, public allowlist, and validated defaults', () => {
  ...
  try {
    writeFileSync(rootFile, [
      'VITE_API_URL=/root-api',
      'API_PORT=18000',
      'FLASHCARD_AI_API_KEY=private-flashcard-file',
      'RAG_AI_API_KEY=private-answer-file',
      'RAG_EMBEDDING_API_KEY=private-embedding-file',
      'GEMINI_API_KEY=removed-legacy-file',
      '',
    ].join('\n'))
    ...
    const privateProcessValues = {
      FLASHCARD_AI_API_KEY: 'private-flashcard-process',
      RAG_AI_API_KEY: 'private-answer-process',
      RAG_EMBEDDING_API_KEY: 'private-embedding-process',
      GEMINI_API_KEY: 'removed-legacy-process',
      RAG_ENABLED: 'true',
    }
    assert.deepEqual(publicEnvironment(rootFile, privateProcessValues), {
      apiUrl: '/root-api', apiPort: 18000,
    })
    const publicPayload = JSON.stringify(publicEnvironment(rootFile, privateProcessValues))
    for (const privateValue of Object.values(privateProcessValues)) {
      assert.equal(publicPayload.includes(privateValue), false)
    }
  } finally {
