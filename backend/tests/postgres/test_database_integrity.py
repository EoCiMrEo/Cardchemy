"""PostgreSQL-only proof for the database schema and transaction contract."""
...
async def seed_course(
    ...
    *,
    ...
    flashcard_set = FlashcardSet(subject=subject, title="database", is_published=False)
    cards = [
        Flashcard(
...
async def test_concurrent_same_student_invitation_replay_is_idempotent(postgres_session_factory):
    nonce = uuid4().hex
    owner = User(
...
async def test_migration_installs_timestamptz_constraints_and_indexes(postgres_session_factory):
    async with postgres_session_factory() as session:
...
async def test_database_rejects_invalid_bounds_and_flashcards(postgres_session_factory):
    course = await seed_course(postgres_session_factory)
    now = utcnow()
...
async def test_publish_trigger_preserves_at_least_one_approved_card(postgres_session_factory):
    course = await seed_course(postgres_session_factory, card_count=0)
...
async def test_account_and_subject_graph_cascades_preserve_invite_history(postgres_session_factory):
    course = await seed_course(postgres_session_factory, published=True)
    nonce = uuid4().hex
    consumer = User(
...
async def test_direct_subject_delete_cascades_all_subject_dependents(postgres_session_factory):
    course = await seed_course(postgres_session_factory, published=True)
    now = utcnow()
    async with postgres_session_factory() as session:
        async with session.begin():
...
async def test_duplicate_enrollment_constraint_is_race_safe(postgres_session_factory):
    course = await seed_course(postgres_session_factory)
    nonce = uuid4().hex
    racing_student = User(
...
async def test_concurrent_progress_updates_are_lossless(postgres_session_factory):
    course = await seed_course(postgres_session_factory, published=True)
    ready = asyncio.Event()
    arrived = 0
...
async def test_concurrent_duplicate_answer_replays_one_durable_receipt(postgres_session_factory):
    course = await seed_course(postgres_session_factory, published=True)
...
async def test_failed_generation_and_progress_batches_leave_no_partial_records(postgres_session_factory):
    course = await seed_course(postgres_session_factory, card_count=2, published=True)
