...
async def generate_cards_from_chunk(state: Dict) -> Dict:
    ...
    We will assume specific structure passed by Send() or Map: {'chunk': str}
    """
    ...
    prompt = PromptTemplate(
        template="""
        You are a flashcard generator. Create high-quality flashcards based *only* on the provided text context.
...
async def reduce_and_review(state: AgentState) -> Dict:
    ...
    print(f"🔄 Reducing {len(all_cards)} cards...")
    ...
    print(f"✅ Finalizing {len(final_list)} unique cards.")
